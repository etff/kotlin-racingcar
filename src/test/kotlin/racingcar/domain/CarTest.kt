...
internal class CarTest {
    ...
    @Test
    fun `자동차는 위치를 가지고 있다`() {
        val givenPosition = Position.valueOf(2)
    ...
    fun `자동차의 위치가 0보다 작으면 예외를 던진다`() {
        assertThatCode { Car(Name("test"), Position.valueOf(-1)) }.isInstanceOf(IllegalArgumentException::class.java)
    }

    @Test
    fun `자동차를 앞으로 이동할 수 있다`() {
        // given
        val car = Car(Name("test"), Position.valueOf(0), FixMoveStrategy())
        // when
    ...
    fun `자동차 위치와 비교할 위치값을 비교하여 최대값을 리턴한다`() {
        val car = Car(Name("test"), Position.valueOf(1))
    ...
    fun `자동차 위치와 최대값이 같으면 true를 리턴한다`() {
        val car = Car(Name("test"), Position.valueOf(5))
